...
class letter_ai(Document):
    def validate(self):
        ...
        # --- (2) If user provided Jalali (preferred input), parse & set Date
        if jalali_text:
            greg_iso = parse_jalali_to_greg_iso(jalali_text)  # raises on bad input
            self.ref_date = greg_iso                         # <- Date field expects 'YYYY-MM-DD'
            # Normalize/echo back a canonical Jalali string
